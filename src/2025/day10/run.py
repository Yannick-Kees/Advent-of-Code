...
class day(riddle):
    ...
    def a(self):
        ...
        for line in tqdm(self.lines):
            ...
            line = line.split(" ")
            light = Light(list(map(int, line[0][1:-1].replace("#", "1").replace(".", "0"))))
            ...
            while not found_solution:
                ...
                for my_actions in product(possible_actions, repeat=my_repeat):
                    test_light = Light(light.value.copy())
    ...
    def b(self):
        fewest_clicks = []
        with Path.open("day10/test.txt") as f:
            for line in tqdm(f.read().splitlines()):
                ...
                light = Light(
                    list(map(int, line[0][1:-1].replace("#", "1").replace(".", "0"))),
                    goal=list(map(int, goal[1:-1].split(","))),
                )
                ...
                while not found_solution:
                    ...
                    for my_actions in product(possible_actions, repeat=my_repeat):
                        test_light = Light(light.value.copy(), light.original_goal.copy())
